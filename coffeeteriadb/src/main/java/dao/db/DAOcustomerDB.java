...
public class DAOcustomerDB {
    ...
    public DAOcustomerDB(Configuration config) {
        this.config = config;
    }
    ...
    public Either<ErrorCCustomer, List<Customer>> getAll() {
        ...
        try (Connection myConnection = DriverManager.getConnection(config.getPropertySQL("pathSQL"), config.getPropertySQL("userSQL"), config.getPropertySQL("passSQL"));
