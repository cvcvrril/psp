...
public class DaoRegistroImp implements DaoRegistro {
    ...
    public Either<ApiError, Usuario> get(int id) {
        ...
        try (Connection myConnection = db.getConnection()){
            PreparedStatement pstmt = myConnection.prepareStatement("select * from credenciales where id =?");
            pstmt.setInt(1, id);
    ...
    public Either<ApiError, Integer> add(Usuario nuevoUsuario) {
        ...
        try (Connection myConnection = db.getConnection()){
            ...
            try (PreparedStatement pstmt = myConnection.prepareStatement("insert into credenciales (usuario, pass) values (?, ?)", Statement.RETURN_GENERATED_KEYS )){
